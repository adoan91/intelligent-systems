...
int decode(int index, int start)
{
  ...
  for (int i=0; i<CHROM_LENGTH; i++)
    value += (int) pow(2.0,(double)i) * pool[index].string[(((CHROM_LENGTH*2)-1) - (15 - start))-i];
  return (value);
}
...
double evaluate(int valueX, int valueY)
{
  double d = convRange(valueX);
  double f = convRange(valueY);
  double g = (double) ((d * d) - (f * f)) + (500 * sin(d));//(0.1*(fabs(d))-sin(f));
  return(g);
}
...
void statistics()
{
  ...
  for (i=0; i<POPULATION_SIZE; i++)
  {
    ...
    cout << "\t" << convRange(decode(i, 1))<< "\t" << convRange(decode(i, 16));
  }
  cout << endl;
}
